    ...
    methods: {
        intall: function () {
            ...
            $(this.$el).daterangepicker({
                ...
                var startDate = moment(start).format(defaultdate);
                vm.$emit('input',startDate);
            });
        },
    },
    mounted: function() {
...
Vue.component('form_singledatepicker', {
    template: `
    <div class="form-group">
    <label v-html="label+':'"></label>
    <input type="text" id="title" v-model="val" :value="val" class="form-control form-control-sm">
    ...
    methods: {
        intall: function () {
            ...
            $(this.$el).daterangepicker({
                ...
                var startDate = moment(start.format(vm.format), vm.format);
                vm.$emit('input',startDate._i);
            });
            $(this.$el).on('apply.daterangepicker', function(ev, picker) {
                var startDate = moment(picker.startDate.format(vm.format), vm.format);
                vm.$emit('input',startDate._i);
            })
        },
        onBlur() {
...
Vue.component('form_customer', {
    template: `
    <div class="form-group">
        <label v-if="label" v-html="label"></label>
        <select v-model="val" :id="id" :data-placeholder="placeholder" class="form-control form-control-sm  select-remote-data-customer">
    </select>
    </div>`,
    ...
    methods: {
        init: function () {
            ...
            if (vm.val) {
                $.ajax({
                    url: '/tkadmin/customers/find/' + vm.val,
                    type: 'GET',
                    dataType: 'json',
                }).done(function(res) {
                    if (res.success) {
                        var option = new Option(res.customer.fullname + ' - ' + res.customer.email + ' - ' + res.customer.phone, res.customer.id, true, true);
                        $('#' + vm.id).append(option);
                    }
                });
            }
            ...
            var config = {
                ...
                templateResult: function(container) {
                    if (container.loading) return container.text;
                    return '<div class="select2-result-containersitory__title">' + container.fullname + ' - ' + container.email + ' - ' + container.phone + '</div>';
                },
                templateSelection: function(repo) {
                    return repo.fullname ? (repo.fullname + ' - ' + repo.email + ' - ' + repo.phone) : repo.text;
                },
            };
            $('#' + vm.id).select2(config)
            .on('change', function(e){
...
Vue.component('form_product', {
    template: `
    <div class="form-group">
        <label v-if="label" v-html="label"></label>
        <select v-model="val" :id="id" :data-placeholder="placeholder" class="form-control form-control-sm  select-remote-data-products">
    </select>
    </div>`,
    props: ['label', 'value', 'placeholder'],
    data: function () {
    ...
    methods: {
        init: function () {
            ...
            if (vm.val) {
                $.ajax({
                    url: '/tkadmin/golf/products/find/' + vm.val,
                    type: 'GET',
                    dataType: 'json',
                }).done(function(res) {
                    if (res.success) {
                        var option = new Option(res.product.code + ' - ' + res.product.title + ' - ' + res.product.price_text, res.product.id, true, true);
                        $('#' + vm.id).append(option);
                    }
                });
            }
            ...
            var config = {
                ...
                allowClear : true,
                ajax: {
                    url: '/tkadmin/golf/products/search',
                    dataType: 'json',
                    delay: 250,
                    type: "get",
                    data: function(params) {
                ...
                templateResult: function(container) {
                    if (container.loading) return container.text;
                    return '<div class="select2-result-containersitory__title">' + container.code + ' - ' + container.title + ' - ' + container.price_text + '</div>';
                },
                templateSelection: function(repo) {
                    return repo.code ? (repo.code + ' - ' + repo.title + ' - ' + repo.price_text) : repo.text;
                },
            };
            $('#' + vm.id).select2(config)
            .on('change', function(e){
...
Vue.component('form_order', {
    ...
    data: function () {
        return {
            val: this.value,
            id: 'order'+Math.floor((Math.random() * 20) + 1)
        }
    },
    methods: {
        init: function () {
            ...
            if (vm.val) {
                $.ajax({
                    url: '/tkadmin/golf/orders/find/' + vm.val,
                    type: 'GET',
                    dataType: 'json',
                }).done(function(res) {
                    if (res.success) {
                        var option = new Option(res.order.code + ' - ' + res.order.fullname + ' - ' + res.order.phone, res.order.id, true, true);
                        $('#' + vm.id).append(option);
                    }
                });
            }
            ...
            var config = {
                ...
                allowClear : true,
                ajax: {
                    url: '/tkadmin/golf/orders/search',
                    dataType: 'json',
                    delay: 250,
                    type: "get",
                    data: function(params) {
                ...
                templateResult: function(container) {
                    if (container.loading) return container.text;
                    return '<div class="select2-result-containersitory__title">' + container.code + ' - ' + container.fullname + ' - ' + container.phone + '</div>';
                },
                templateSelection: function(repo) {
                    return repo.code ? (repo.code + ' - ' + repo.fullname + ' - ' + repo.phone) : repo.text;
                },
            };
            $('#' + vm.id).select2(config)
            .on('change', function(e){
