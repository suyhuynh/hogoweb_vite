...
class Customer {
    ...
    public static function getAllCustomerCode() {
        $sql = mysqli_query(dbconnect(),"SELECT `code`, `fullname`, `phone` FROM `customer_gullivers` WHERE `status` = 'active'");
    ...
    public static function getCustomerById($id) {
        $sql = mysqli_query(dbconnect(),
            "SELECT * FROM `customer_gullivers` WHERE `id` = $id"
        );
    ...
    public static function getCustomerByEmailOrPhone($phone, $email) {
        $sql = mysqli_query(dbconnect(),
            "SELECT * FROM `customer_gullivers` WHERE `phone` = '" . trim($phone) . "' OR `email` = '" . trim($email) . "'"
        );
		$result = mysqli_fetch_assoc($sql) ;
		return $result;
    }
    ...
    public static function getCustomerByCode($code) {
        $sql = mysqli_query(dbconnect(),
            "SELECT * FROM `customer_gullivers` WHERE `code` = '" . $code . "'"
        );
		$result = mysqli_fetch_assoc($sql) ;
		return $result;
    }
    ...
    private static function create($data) {
        ...
        $sql = "INSERT INTO `customer_gullivers` 
		(
            `" . implode("`, `", $keys) . "`
		)
		VALUE 
