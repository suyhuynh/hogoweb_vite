...
class Customer {
    ...
    public static function getAllCustomerCode() {
        $sql = mysqli_query(dbconnect(),"SELECT `code`, `fullname`, `phone` FROM `customer_gulliver_fanmeeting_surveys` WHERE `status` = 'active'");
    ...
    public static function getAllCustomerCodeFull() {
        $sql = mysqli_query(dbconnect(),"SELECT `code`, `fullname`, `phone` FROM `customer_gulliver_fanmeeting_surveys`");
    ...
    public static function getCustomerById($id) {
        $sql = mysqli_query(dbconnect(),
            "SELECT * FROM `customer_gulliver_fanmeeting_surveys` WHERE `id` = $id"
        );
    ...
    public static function getCustomerByEmailOrPhone($phone, $email) {
        $sql = mysqli_query(dbconnect(),
            "SELECT * FROM `customer_gulliver_fanmeeting_surveys` WHERE `phone` = '" . trim($phone) . "' OR `email` = '" . trim($email) . "'"
        );
		$result = mysqli_fetch_assoc($sql) ;
		return $result;
    }
    ...
    public static function getCustomerByCode($code) {
        $sql = mysqli_query(dbconnect(),
            "SELECT * FROM `customer_gulliver_fanmeeting_surveys` WHERE `code` = '" . $code . "'"
        );
		$result = mysqli_fetch_assoc($sql) ;
		return $result;
    }
    ...
    public static function setStatus($id) {
        ...
        $sql = "UPDATE `customer_gulliver_fanmeeting_surveys` 
			SET `updated_at` = '$now', `status` = 'active'
			WHERE `id` = '" . $id . "'";
        return mysqli_query(dbconnect(),$sql);
    }

    public static function updateLuckyCode($luckyCode) {
        $now = date('Y-m-d H:i:s');
        $sql = "UPDATE `customer_gullivers` 
			SET `updated_at` = '$now', `status` = 'lucky'
    ...
    private static function create($data) {
        ...
        $sql = "INSERT INTO `customer_gulliver_fanmeeting_surveys` 
		(
            `" . implode("`, `", $keys) . "`
		)
		VALUE 
