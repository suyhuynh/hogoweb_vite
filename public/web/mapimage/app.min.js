    ...
    var b = 0,
        ...
        d = {
            defaults: {
                ...
                input: {
                    active: {
                        selector: 'input[dataname^="im["][dataname$="][active]"]',
                        fn: function () {
                            console.log(a(this).attr("dataname").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1));
                            return a(this).attr("dataname").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                        }
                    },
                    shape: {
            ...
            addArea: function (b, d) {
                ...
                c[b].state.areaLength = (area_item.length >= 1) ? area_item.length : c[b].state.areaLength + 1, 
            ...
            asHTML: function (b) {
                ...
                var d = a("<img />"),
                    e = c[b].options.src;
                c[b].options.file.length > 0 && (e = c[b].options.file), d.attr("src", e), d.attr("usemap", "#image-map"+time), d.attr("class", "img-responsive img-map");
                var f = a("<map />");
                f.attr("name", "image-map"+time);
                f.attr("class", "image-map");
                var g = [];
    ...
    a(document).on("init", function () {
        ...
        a(document).on("click", "button.add-row", function (b) {
            ...
            var b = a(this).imageMapper("getData"),
                ...
                d = imageMapperRow;
                console.log(b);
                if((b.area).length <= 1 && area_item.length) {
                    b.area = area_item;
                }
                let data_area = [];
                (b.area).forEach(element => {
                    delete element.el
                    data_area.push(element);
                });
                $('#data-json-mapper').val(JSON.stringify(data_area));
        ...
        }), a(document).on("click", "#get-code", function () {
            var b = a("#image-map").imageMapper("asHTML");
            a("#modal-code-result").text(b).css("whiteSpace", "pre")
