	...
	<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
		<div class="d-flex">
			<div class="breadcrumb">
				...
				<span class="breadcrumb-item active">
					{{ trans('core::functions.group.'.request()->code) }}
				</span>				
			</div>
		...
		<div class="header-elements d-none">
			<div class="breadcrumb justify-content-center">
				<a href="{{ route('admin.groups.create', ['code' => request()->code]) }}" class="btn {{ config('erp.btn_class.create.class') }} btn-sm btn-actions btn-create" style="margin-left: 5px;">
            ...
            <div class="card-heading">
                <div class="row">
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <p style="margin-top: 8px;margin-bottom: 0;text-transform: uppercase;"><strong>{{ trans('core::functions.group.'.request()->code) }}</strong></p>
                    </div>
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <div class="row">
                ...
                <table class="table-hover table">
                    <thead>
                        <tr>
                            ...
                            <th>{{ trans('website::posts.table.img') }}</th>
                            <th>{{ trans('core::categorys.table.title') }}</th>
                            <th>{{ trans('core::categorys.table.description') }}</th>
                            <th>{{ trans('website::layouts.form.design') }}</th>
                    ...
                    <tbody>
                        <tr v-if="!isLoading" v-for="(item, key) in data">
                            ...
                            <td>
                                <template v-if="item.translate.img">
                                    <img :src="item.translate.img" class="img-responsive">
                                </template>
                            </td>
                            <td v-html="item.translate.title"></td>
                            <td v-html="item.translate.description"></td>
                            <td v-html="item.url_design"></td>
                            ...
                            <td>
                                ...
                                &nbsp;
                                <a :href="'{{ route("admin.groups.edit", ['id' => '', 'code' => request()->code]) }}/'+item.id" class="text-primary" v-tooltip title="{{ trans('resource.edit') }}">
    ...
    var mix = {
        ...
        methods: {
            load: function(number = null ){
                ...
                helper.post( '{{ route('admin.groups.index', ['code' => request()->code]) }}' , formdata ,15000)
            ...
            updateStatus: function(status) {
                ...
                helper.post( '{{ route("admin.groups.status", ['code' => request()->code]) }}' , formdata ,15000)
                .done( function(res , status , xhr){
                    vm.isLoading = false;
                    if(res.success){
                        vm.load();
            ...
            destroy: function(id = '') {    
                ...
                var callbacktrue = function(){
                    ...
                    var formdata = new FormData;
                    formdata.append('ids' , vm.ids.toString());
                    helper.post( '{{ route("admin.groups.destroy", ['code' => request()->code]) }}' , formdata ,15000)
                    .done( function(res , status , xhr){
                        vm.isLoading = false;
                        if(res.success){
                            vm.load();
        ...
        mounted() {
            var _this = this;
            $('table').on('click', '.save-layout', function(event) {
